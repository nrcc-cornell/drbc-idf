...
export const tabsInfo = [
  {
    ...
    "content": 
      <Box key='user-guide' sx={{ padding: '8px' }}>
        ...
        <ParagraphTypography>This site was designed to estimate projected changes in extreme rainfall in the Delaware River Basin. Through the interactive tool, users can see estimated Intensity-Duration-Frequency (IDF) curves across a range of durations from 5 minutes to 24 hours by specifying five factors: 1) county,municipality or watershed (HUC 12) scale; 2) climate model generation; 3) emission scenario; 4) future time period; and 5) annual exceedance probability (frequency of exceedance). By sharing this information in a user-friendly tool, the DRBC is providing local officials, policymakers, planners, and developers, as well as Basin residents, with science-based knowledge to understand expected climate-induced precipitation changes and inform adaptation decisions.</ParagraphTypography>
        ...
        <ParagraphTypography><b>Select By:</b> Changes the selectable areas on the map. Once the map shows the desired area scale, you can click on the map to select a specific location. If you have a location selected and change this option, the tool will automatically determine the new location that should be selected based on the pin location.</ParagraphTypography>
        <ParagraphTypography><b>Climate Model Generation:</b> Changes the ensemble of climate models between CMIP6 and CMIP5.  CMIP6 is the current scientific standard. These models feature higher spatial resolution and updated physics. CMIP5 models were released in 2013. The IDF curves presented on this site previously were based on CMIP5 models.</ParagraphTypography>
        <ParagraphTypography><b>Emission Scenario:</b> Changes the projection based on future emissions.  For CMIP6 models, Shared Socioeconomic Pathways (SSP) are used with options for intermediate (SSP2-4.5), high (SSP3-7.0), and very high (SSP5-8.5) available.  Two Representative Concentration Pathways (RCP) are used for CMIP5 models, these are termed low RCP4.5 and high RCP8.5.</ParagraphTypography>
        <ParagraphTypography><b>Time Period:</b> Changes the projection time period.</ParagraphTypography>
        <ParagraphTypography><b>Annual Exceedance Probability:</b> Changes the probability of exceedance. This percentage represents the probability with which a given storm would occur in a given year. A 50% exceedance probability is often referred to as a 2-year storm, such storms have a 50% chance of occurring in any year. A 20% exceedance probability indicates a 5-year storm.  The 5-year storm has a 20% chance of occurring in any given year.  The 10%, 4%, 2% and 1% exceedance probabilities correspond to a 10-year, 25-year, 50-year and 100-year storm, respectively.  An important clarification is that a 100-year storm has a 1% chance of occurring in any year.  It does not mean that such an event occurs only once in 100 years.</ParagraphTypography>
        ...
        <ParagraphTypography>A new table will populate at the bottom of the map and display the change factors for the area that you selected.</ParagraphTypography>
  ...
  },
  {
    ...
    "content": 
      <Box key='about-the-data' sx={{ padding: '8px' }}>
        ...
        <ParagraphTypography>The historical and future downscaled atmosphere–ocean general circulation model (AOGCM) and Earth System model (ESM) output used in this study was obtained from three sources.  The new CMIP6 change factors are based on statistically downscaled projections from the Localized Constructed Analog V2 (LOCA2) dataset (Pierce et al., 2023). For these, simulations from the models’ historical period and three Shared Socioeconomic Pathways (SSP2-4.5, SSP3-7.0 and SSP5-8.5) were used (Riahi et al., 2017). The original CMIP5 change factors use two downscaled datasets from 1) North American Coordinated Regional Downscaling Experiment (NA-CORDEX) (Mearns et al., 2017) and 2) Localized Constructed Analog (LOCA) (Pierce et al., 2014) and are presented for two future Representative Concentration Pathways (RCP4.5 and RCP 8.5) (Collins et al. 2013).</ParagraphTypography>
        
        <ParagraphTypography>Table 1 highlights the unique features of each of these datasets. For example, LOCA2 downscaling provides one of the highest resolution outputs, while NA-CORDEX, a dynamically downscaled dataset, has a much coarser spatial resolution. Since the downscaling approach, choice of global climate model (GCM), emissions scenario (SSP or RCP), and spatial and temporal resolution each introduce their own sources of uncertainty, it is important to consider a range of these features to capture the plausible range of future precipitation extremes. Unfortunately, NA-CORDEX downscaling for CMIP6 is not available, so CMIP6 change factors are based solely on LOCA2.</ParagraphTypography>

        <ParagraphTypography>To assess the impact of not including the NA-CORDEX simulations, CMIP5 change factors using only LOCA data were re-computed. This analysis showed that the omission had minimal effect. Two other downscaled data sets were considered when computing the original CMIP5 change factors. Bias Corrected Constructed Analogues (BCCAv2) (Maraun et al., 2010) and Multivariate Adaptive Constructed Analogs (MACA) (Abatzoglou and Brown, 2012) but were excluded from the study based on the results of Lopez-Cantu et al. (2020).</ParagraphTypography>
        ...
        <figure style={{ margin: '12px 25px' }}>
          ...
          <div></div>
          <img src={`${process.env.PUBLIC_URL}/assets/ensemble_characteristics.png`} alt='Table describing characteristics of downscaled climate model datasets used in this study' style={{ width: '100%' }} />
        ...
        <ParagraphTypography>The LOCA2 dataset is widely used as it forms the basis for downscaling in the current 5th National Climate Assessment (USGCRP, 2023). Projections from the 16 GCMs summarized in Table 2 were used. The downscaled LOCA projections have a spatial resolution of 0.0625° (approximately 6 km or the distance from Philadelphia to Camden). Details regarding the statistical downscaling methodology used in LOCA can be found in Pierce et al. (2014). Pierce et al. (2023) provides details about differences in the LOCA2 methodology. Briefly, the method begins by matching the spatial pattern of the variable of interest from a future GCM projection to that based on historical observations over a region. From a pool of candidate observed historical analog days the single best candidate, based on minimization of root mean square error, is chosen as an analog, unless a different analog day is selected for neighboring grid cells, in which case a weighted combination of the observed analog days is used. In either case, high-resolution observed data corresponding to the historical analog day is used to represent the downscaled future rainfall.</ParagraphTypography>

        <figure style={{ margin: '12px 25px' }}>
          <TableHeadingTypography><b>Table 2.</b> List of CMIP6 models used in LOCA2 downscaling</TableHeadingTypography>
          <img src={`${process.env.PUBLIC_URL}/assets/cmip6_models.png`} alt='Table describing CMIP6 models used in LOCA2 downscaling' style={{ width: '100%' }} />
        </figure>

        <ParagraphTypography>For the CMIP5 data, LOCA was also used. It formed the basis for downscaling in the 4th National Climate Assessment (USGCRP, 2018). In this case, projections from the 31 GCMs summarized in Table 3 were used. NA-CORDEX, on the other hand, employs a dynamical downscaling approach based on a set of regional climate models (RCM) with boundary conditions specified by GCM simulations from the CMIP5 archive. The RCM domain is limited to the area covering the majority of North America. Like LOCA, historical and future simulations are available for the period 1950-2100, using both RCP8.5 and RCP4.5. Most simulations are available with a spatial resolution of 0.22° (approximately 25 km or half the distance from Philadelphia to Trenton), however a subset of RCP4.5 simulations are only available at a 0.44° (50 km) resolution (the full distance from Philadelphia to Trenton). Table 4 summarizes the NA-CORDEX simulations used.</ParagraphTypography>

        <figure style={{ margin: '12px 25px' }}>
          <TableHeadingTypography><b>Table 3.</b> List of CMIP5 models used in LOCA downscaling</TableHeadingTypography>
          <img src={`${process.env.PUBLIC_URL}/assets/cmip5_models.png`} alt='Table describing CMIP5 models used in LOCA downscaling' style={{ width: '100%' }} />
          <CaptionTypography>*projection only</CaptionTypography>
        </figure>
        
        <figure style={{ margin: '12px 25px' }}>
          <TableHeadingTypography><b>Table 4.</b> NA-CORDEX simulations used in CMIP5 downscaling</TableHeadingTypography>
          <img src={`${process.env.PUBLIC_URL}/assets/na-cordex_simulations.png`} alt='Table describing NA-CORDEX simulations used in CMIP5 downscaling' style={{ width: '100%' }} />
          <CaptionTypography>*GEMatm is a global version of the CRCM5 regional model (Hernandez-Diaz, 2019)</CaptionTypography>
          <CaptionTypography><sup>a</sup>RegCM4: Regional Climate Model System, National Center for Atmospheric Research, USA</CaptionTypography>
          <CaptionTypography><sup>b</sup>CRCM4 and CRCM5 Canadian Regional Climate Model, Canadian Centre for Climate Modeling and Analysis, Canada*.  Complementary Simulations available from Université du Québec à Montréal (UQAM)and OURANOS, Canada.</CaptionTypography>
          <CaptionTypography><sup>c</sup>RCA4: Rossby Centre regional climate model, Rosby Centre, Sweden</CaptionTypography>
          <CaptionTypography><sup>d</sup>HIRAM5 High Resolution Atmospheric Model, Geophysical Fluid Dynamics Lab, USA</CaptionTypography>
          <CaptionTypography><sup>e</sup>WRF: Weather Research and Forecasting Model National Center for Atmospheric Research, and others, USA</CaptionTypography>
        </figure>
        ...
        <figure style={{ margin: '12px 25px' }}>
          <img src={`${process.env.PUBLIC_URL}/assets/workflow.png`} alt='Figure describing workflow used in analysis' style={{ width: '100%' }} />
        ...
        <ParagraphTypography>A PDS was compiled for each grid cell, corresponding to either the 50-year period from 1950- 1999 for grids outside of New York State or the 64-year period from 1950-2013 at New York locations. These periods were designated the historical period as they correspond to the longest available overlap period between the downscaled historical simulations and stations included in either NOAA Atlas 14 Vol 2 or, for New York, NOAA Atlas 14 Vol 10 (Perica et al., 2019).  The longer PDS period for New York was used only in the CMIP6 simulations.</ParagraphTypography>
        <ParagraphTypography>PDS for two future time periods 2020-2069 and 2050-2099 were also extracted in all cases. The 50-year length was selected to assure an adequate sample size for extreme value analysis, and to minimize the influence of the non-stationarity of the record and the potential effect of natural interdecadal variations in the extreme rainfall record (DeGaetano and Castellano, 2017). Separate PDS were compiled for simulations using SSP2-4.5, SSP3-7.0 and SSP5-8.5 in CMIP6 and for RCP8.5 and RCP4.5 when CMIP5 was used.</ParagraphTypography>
        ...
        <ParagraphTypography>As outlined in Figure 1, the development of 2-, 5-, 10-, 25-, 50- and 100-year recurrence interval precipitation amounts using L-moments fitting of the GEV distribution is the second of a five-part workflow. In the third step an ensemble of change-factors was computed for each gridpoint-model-scenario combination. Change-factors (CF) are defined as:</ParagraphTypography>
        ...
        <ParagraphTypography>In the final step, the uncertainty of the CFs was quantified. At each 0.1° grid point an ensemble of 16,000 LOCA2-CMIP6 CFs was available for each recurrence-interval-SSP-combination (16 models x 1000 resamples) For the CMIP5 simuations, as ensemble of 47,000 CFs was available for each recurrence interval under RCP8.5 (47 models; 31 LOCA and 16 CORDEX x 1000 resamples) and 40,000 CFs existed under RCP4.5. From these ensembles the median CF factor was obtained as were the 17th, 25th, 75th and 83rd percentiles. The 17th and 83rd percentiles were selected to represent a likely range (Mastrandrea et al. 2010) of projections around the median.</ParagraphTypography>
        ...
        <CitationTypography>
          Riahi, K., D.P. van Vuuren, E. Kriegler, J. Edmonds, B.C. O’Neill, S. Fujimori, N. Bauer, K. Calvin, R. Dellink, O. Fricko, W. Lutz, 2017.
        ...
        <CitationTypography>
          USGCRP, 2023: Fifth National Climate Assessment. Crimmins, A.R., C.W. Avery, D.R. Easterling, K.E. Kunkel, B.C. Stewart, and T.K. Maycock, Eds. U.S. Global Change Research Program, Washington, DC, USA. <a href="https://doi.org/10.7930/NCA5.2023" target='_blank' rel='noreferrer'>https://doi.org/10.7930/NCA5.2023</a>
        </CitationTypography>
